...
class User(UserMemberType):
    ...
    def login(self, hostId):
        return self.gizmo.session.post(
            f'{self.gizmo.apiUri}/api/users/{self.id}/login/{hostId}'
        )
    ...
    def _api_update_value(self, name, value):
        res = self.gizmo.session.post(
            f'{self.gizmo.apiUri}/api/users/',
            params={
                'UserId': self.id,
        ...
        def checkReservation():
            ...
            for reservation in reservations:
                if reservation.status == 1:
                    continue
                if host.id not in [host.id for host in reservation.hosts]:
                    continue
